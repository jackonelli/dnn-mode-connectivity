...
if args.dataset is not None:
    ...
    num_classes = 10
    loader = [(torch.randn((args.batch_size, 3, 32, 32)), None)
              for i in range(20)]
...
base = [
    architecture.base(num_classes, **architecture.kwargs) for _ in range(2)
]
for base_model, path, k in zip(base, [args.init_start, args.init_end],
                               [0, args.num_bends - 1]):
    if path is not None:
